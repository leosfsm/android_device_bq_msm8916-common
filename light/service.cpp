...
#define LOG_TAG "android.hardware.light@2.0-service.bq_8916"
...
int main() {
    ...
    std::ofstream buttonbacklight(kButtonlightPath);
    if (!buttonbacklight) {
        LOG(ERROR) << "Failed to open " << kButtonlightPath << ", error=" << errno
                   << " (" << strerror(errno) << ")";
        return -errno;
    }
    ...
    android::sp<ILight> service = new Light(
            std::move(backlight), std::move(buttonbacklight),
